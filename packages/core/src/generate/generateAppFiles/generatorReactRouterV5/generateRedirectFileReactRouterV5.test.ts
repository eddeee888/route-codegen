    ...
    expect(templateFile.template).toMatchInlineSnapshot(`
      "import React from 'react'
        ...
        export const RedirectLogin: React.FunctionComponent<UrlPartsLogin & { fallback?: React.ReactNode }> = props => {
          const to = generateUrl({ pattern: patternLogin, path: {}, query: props.query, origin: props.origin);
          return (
            <>
              <Redirect to={to} />
              {props.fallback}
  ...
  it("should generate correctly with path params", () => {
    ...
    expect(templateFile.template).toMatchInlineSnapshot(`
      "import React from 'react'
        ...
        import {UrlPartsLogin,patternLogin,} from './patternLogin'
        export const RedirectLogin: React.FunctionComponent<UrlPartsLogin & { fallback?: React.ReactNode }> = props => {
          const to = generateUrl({ pattern: patternLogin, path: props.path, query: props.query, origin: props.origin);
          return (
            <>
              <Redirect to={to} />
              {props.fallback}
